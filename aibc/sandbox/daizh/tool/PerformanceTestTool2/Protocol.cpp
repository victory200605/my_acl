...
//////////////////////////////////////////////////////////////////////////////////////////////
int RecvCMPPPacketEx(int aiSocket, int* apoMessageID, CMPPHead* apoCMPPHead, AIChunk* apoAIChunk, int aiTimeout)
{
    size_t      liBodySize;
    ssize_t     liRetCode = 0;
    ...
    *apoMessageID = ntohl(loAsyncHead.ciMsgID);
    
    return  (0);//(apoCMPPHead->ciTotalLength);
}

int SendCMPPPacketEx(int aiSocket, int aiMessageID, CMPPHead const* apoCMPPHead, void const* apvBody, int aiTimeout)
{
    int         liRetCode = 0;
    size_t      liSendSize = 0;
    char*       lpcSendBuf = NULL;
    size_t      liHeaderSize = CMPP_HEADER_LEN + sizeof(stASYNCHEAD);
    CMPPHead    loCMPPHead;
    stASYNCHEAD loAsyncHead;

    liSendSize = apoCMPPHead->ciTotalLength + sizeof(stASYNCHEAD);
    AI_NEW_N_ASSERT(lpcSendBuf, char, liSendSize);
    
    loAsyncHead.ciMsgID = htonl(aiMessageID);
    ...
    if(liSendSize > CMPP_HEADER_LEN)
    {
        if(apvBody != NULL)
        {
            memcpy(lpcSendBuf+liHeaderSize, apvBody, liSendSize-liHeaderSize);
        }
        else
        {
            memset(lpcSendBuf+liHeaderSize, 0, liSendSize-liHeaderSize);
        }
    }
...
{
    static_cast<AICmppChannel*>(apoChannel)->Initialize(this, 0);
    return 0;
}
...
int AICmppProtocol::SendRequest( int aiSocket, int aiTimeout )
{
    ...
    if ( ::SendCMPPPacketEx( aiSocket, 1, &loHeader, loChunk.GetPointer(), aiTimeout ) != 0 )
    {
        return ERROR_SEND;
    }
    ...
    if ( ::RecvCMPPPacketEx( aiSocket, &liMessageID, &loHeader, &loResponse, aiTimeout ) != 0 )
    {
        return ERROR_RECV;
    }
...
int AICmppProtocol::RecvRequest( int aiSocket, int aiTimeout )
{
    ...
    if ( ( liRetCode = ::RecvCMPPPacketEx( aiSocket, &liMessageID, &loHeader, &loChunk, aiTimeout ) ) != 0 )
    {
    ...
    if ( ::SendCMPPPacketEx( aiSocket, liMessageID, &loHeader, loResponse.GetPointer(), aiTimeout ) != 0 )
    {
        return ERROR_SEND;
    }
