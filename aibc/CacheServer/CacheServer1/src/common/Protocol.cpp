...
int clsHeader::GetSize()
{
    if ( this->cpoBody != NULL )
    {
        return this->cpoBody->GetSize() + sizeof(int32_t)*2;
    }
...
int clsRequestPut::Decode(AIBC::AIChunkEx& aoBuffer)
{
	...
	AI_RETURN_IF( -1, (size_t)liKeySize > AI_MAX_KEY_LEN || liKeySize == 0);
	...
	AI_RETURN_IF( -1, (size_t)liValueSize > AI_MAX_VALUE_LEN || liValueSize == 0);
...
int clsRequestPut::Encode(AIBC::AIChunkEx& aoBuffer)
{
    ...
    AI_RETURN_IF(-1, liKeySize == 0 || liValueSize == 0);
...
{
	return sizeof(int32_t)*2 + this->coKey.GetSize() + this->coValue.GetSize();
}
...
{
	return sizeof(int32_t) + this->coKey.GetSize();
}
...
{
	return clsResponse::GetSize() + sizeof(int32_t) + this->coValue.GetSize();
}
/////////////////////////////end clsResponseGet////////////////////
...
{
	return sizeof(int32_t) * 3 + clsResponse::GetSize();
}
