...
//typedef anf::TAsyncClient<acl::CIndexDict> SyncClientType;
typedef anf::TSyncClient<acl::CIndexDict> SyncClientType;
...
void* Worker(void* apoClient)
{
    ...
    acl::CMemoryBlock loBody(giWriteBufferSize - 12);
    apl_int_t liRetCode = 0;
    
    //loRequest.Set(0x1FF, "abcdefg");
    loRequest.Set(0x1F, "abcdefg");

    while(true)
    {
        if ( (liRetCode = lpoClient->Request(loRequest, &lpoResponse, loTimeout) ) < 0)
        {
            printf("Request fail %"APL_PRIdINT"\n", liRetCode );
            //return NULL;
        }
...
int main(int argc, char* argv[])
{
    ...
    //config sync client
    loClient.GetFilterChain()->AddFirst("simple-async-message", new anf::CBencodingMessageFilter);
