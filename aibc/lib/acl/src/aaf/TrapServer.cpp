...
#include "aaf/TrapServer.h"

AAF_NAMESPACE_START

//////////////////////////////////////////////////////////////////////////////////////////////
CTrapServer::CTrapServer(void)
    : mbIsShutdown(true)
    , mpoReactor(NULL)
    , miEvents(0)
...
apl_int_t CTrapServer::Startup( char const* apcConfigFileName )
{
    ...
    {
        apl_errprintf(
            "TrapServer open config %s fail, %s\n",
            apcConfigFileName,
    ...
    //Load Trap config
    if ( loConfig.GetValue("Trap", "ServerIp").IsEmpty() )
    {
        apl_errprintf( "TrapServer's server ip is empty and shall be disabled\n" );
            
        return 0;
    }
    else
    {
        if ( loConfig.GetValue("Trap", "ServerPort").IsEmpty()
        ...
        {
            apl_errprintf( "TrapServer is enabled and CommandID/ServerPort/TableName/ProgramName/Type/Level can't default\n" );
            
            return -1;
        }
        
        this->muCommandID = loConfig.GetValue("Trap", "CommandID").ToInt();
        this->moServerIp = loConfig.GetValue("Trap", "ServerIp").ToString();
        this->mu16ServerPort = loConfig.GetValue("Trap", "ServerPort").ToInt();
    ...
    if ( (liRetCode = this->ConnectToServer() ) != 0 )
    {
        //Ignore
        //return -1;
    }
    ...
    if (this->mpoReactor != NULL)
    {
        ...
        {
            apl_errprintf( "TrapServer open sockpair fail, %s\n", apl_strerror(apl_get_errno()) );
        ...
        if ( this->moThreadManager.Spawn(CTrapServer::Svc, this) != 0 )
        {
            this->mbIsShutdown = true;
            
            apl_errprintf( "TrapServer spawn svc fail, %s\n", apl_strerror(apl_get_errno()) );
...
apl_int_t CTrapServer::Restart(void)
{
    return -1;
}
    
apl_int_t CTrapServer::Shutdown(void)
{
    this->mbIsShutdown = true;
...
void CTrapServer::SetReactor( acl::CReactor* apoReactor )
{
    this->mpoReactor = apoReactor;
}

apl_int_t CTrapServer::Trap( char const* apcTitle, char const* apcFormat, ... )
{
...
apl_int_t CTrapServer::TrapNonBlock( char const* apcTitle, char const* apcFormat, ... )
{
    CTrap   loTrap;
    va_list loVaList;
    ...
    else
    {
        ...
        if (this->moBuffer.GetLength() > 0 && ACL_BIT_DISABLED(this->miEvents, acl::IEventHandler::WRITE_MASK) )
        {
                ...
                acl::IEventHandler::WRITE_MASK ) != 0 )
            {
                ...
                apl_errprintf(
                    "TrapServer register handler fail,%s [errno=%"APL_PRIdINT"]/[event=%"APL_PRIdINT"]\n",
                    apl_strerror(apl_get_errno()),
                    apl_get_errno(),
...
apl_int_t CTrapServer::HandleOutput( apl_handle_t aiHandle )
{
    ...
    if (this->moBuffer.GetLength() == 0)
    {
        ...
        {
            apl_errprintf(
                "TrapServer remove handler fail,%s [errno=%"APL_PRIdINT"]/[event=%"APL_PRIdINT"]\n",
                apl_strerror(apl_get_errno()),
                apl_get_errno(),
    ...
    else
    {
        //Trap HERE: Can only use blocking mode
        apl_errprintf(
            "TrapServer Sock pair exception, write fail, [result=%"APL_PRIdINT"][errno=%"APL_PRIdINT"]/[size=%"APL_PRIuINT"]\n",
            liResult,
            apl_get_errno(),
...
void* CTrapServer::Svc( void* apvServer )
{
    CTrapServer*     lpoServer = static_cast<CTrapServer*>(apvServer);
    apl_ssize_t      liResult = 0;
    acl::CSockStream loPeer;
    ...
    while(!lpoServer->mbIsShutdown)
    {
        ...
        {
            lpoServer->CheckConnection();
            
            apl_memcpy(&loTrap, lacBuffer, sizeof(loTrap) );
            if ( lpoServer->Trap(loTrap.macTitle, "%s", loTrap.macContent) != 0 )
            {
                //Ignore
            }
        ...
        else
        {
            ...
            else
            {
                //Trap HERE: Can only use blocking mode
                apl_errprintf(
                    "TrapServer Sock pair exception, recv fail, [result=%"APL_PRIdINT"][errno=%"APL_PRIdINT"]\n",
                    liResult,
                    apl_get_errno() );
            }
        }
...
void CTrapServer::CheckConnection(void)
{
    if (this->moPeer.GetHandle() == ACL_INVALID_HANDLE)
    {
...
apl_int_t CTrapServer::ConnectToServer(void)
{
    acl::CSockAddr loAddr(this->mu16ServerPort, this->moServerIp.c_str() );
    ...
    {
        apl_errprintf(
            "TrapServer connect to server [%s:%"APL_PRIdINT"] fail, %s [errno=%"APL_PRIdINT"]\n",
            this->moServerIp.c_str(),
            (apl_int_t)this->mu16ServerPort,
            apl_strerror(apl_get_errno()),
            apl_get_errno() );
