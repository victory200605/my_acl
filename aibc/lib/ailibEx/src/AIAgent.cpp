...
int AIAgent::StartUp(char *apcLocalIp, int aiLocalPort, int aiTimeout)
{
    ...
    if((ciSockfd = ai_socket_create(AF_INET,SOCK_DGRAM,0)) == AI_SOCK_ERROR_SOCKET)
    {
        return AI_SNMP_ESOCKET;
    }
    ai_socket_set_nonblocking(ciSockfd, true);
    ...
    int liFlag = 1;
    setsockopt(ciSockfd, SOL_SOCKET, SO_REUSEADDR, (char*)&liFlag, sizeof(int));
...
int AIAgent::ProcessFunc()
{
    ...
    char lpcRemoteIP[16] ;
    memset(&lpcRemoteIP, 0, sizeof(lpcRemoteIP));
    int liRemotePort = 0;

    if(SnmpAgentRecvPdu(ciSockfd, lpoPdu, lpcRemoteIP, &liRemotePort, ciTimeout) < 0)
    {
    ...
    int liSend = -1;
    if((liSend = SnmpSendPdu(ciSockfd, lpoPdu,(char const *)lpcRemoteIP,liRemotePort)) == EMSGSIZE)
    {   
    //if the pdu can't contain all data
        ...
        SnmpSendPdu(ciSockfd, lpoPdu,(char const *)lpcRemoteIP,liRemotePort);
