    ...
    acl::CTimeValue const& aoTimeout  )
{
    ...
    {
        apl_errprintf( "Async client send request fail, (RetCode=%"APL_PRIdINT",CommandID=0x%X)\n", liRet, aiRequestCmd);
...
bool CErrorCache::IsHitCache()
{
        ...
        || this->miErrNo == ERROR_NO_ENOUGH_SPACE ) )
    {
        ...
        if (loCurrTime.Sec() - this->moTimestamp.Sec() < this->miTimeInterval)
        {
            return true;
        }
    }
...
apl_int_t CGFQueueClient::Get( const char* apcQueueName, acl::CMemoryBlock* apoData, apl_int_t aiWhenSec /* = 0 */)
{
	apl_int_t    liRet = NO_ERROR;
...
apl_int_t CGFQueueClient::GetStat( const char* apcQueueName, apl_int_t* apiSize, apl_int_t* apiCapaciy )
{
...
apl_int_t CGFQueueClient::GetWriteSegment( const char* apcQueueName, CSegmentInfo& aoSegmentInfo )
{
    apl_int_t liRet = NO_ERROR;
	...
	
	{
		...
		if (liRet != NO_ERROR)
		{
		    ...
		    aoSegmentInfo.moErrorCache.moTimestamp.Update();
...
apl_int_t CGFQueueClient::GetReadSegment( const char* apcQueueName, CSegmentInfo& aoSegmentInfo )
{
	apl_int_t liRet = NO_ERROR;
...
apl_int_t CGFQueueClient::GetWriteSegmentSecond( const char* apcQueueName, apl_int_t aiRetcode, CSegmentInfo& aoSegmentInfo )
{
	apl_int_t liRet = NO_ERROR;
...
apl_int_t CGFQueueClient::GetReadSegmentSecond( const char* apcQueueName, apl_int_t aiRetcode, CSegmentInfo& aoSegmentInfo )
{
	apl_int_t liRet = NO_ERROR;
...
apl_int_t CGFQueueClient::SetSegmentInfo( CSegmentInfoMap& aoSegmentInfoMap, const char* apcQueueName, CSegmentInfo& aoSegmentInfo )
{
	// lock scope
...
apl_int_t CGFQueueClient::GetSegmentInfo( CSegmentInfoMap& aoSegmentInfoMap, const char* apcQueueName, CSegmentInfo& aoSegmentInfo )
{
	// lock scope
    acl::TSmartLock<acl::CLock> loGuard(aoSegmentInfoMap.moLock);
...
apl_int_t CGFQueueClient::CreateSegmentServerConnection( const char* apcServerIp, apl_int_t aiPort, ClientType** appoSegmentServer )
{
	// lock scope
